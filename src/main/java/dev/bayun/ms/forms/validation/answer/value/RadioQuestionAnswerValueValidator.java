 ...
 */
public class RadioQuestionAnswerValueValidator implements AnswerValueValidator {

    @Override
    @SuppressWarnings("unchecked")
    public void validate(Object value, @NonNull Question question) throws InvalidAnswerValueException {
        if (!(question instanceof RadioQuestion radioQuestion)) {
            return;
        }

        if (radioQuestion.isRequired() && value == null) {
        ...
        if (value != null) {
            ...
            boolean questionContainsOptionById = radioQuestion.getOptions().stream().anyMatch(option -> option.getId().equals(optionId));
            if (!questionContainsOptionById) {
                throw new InvalidAnswerValueException(String.format("%s doesn't have an option with provided optionId (%s)", radioQuestion, radioValue));
            }
        }
    }
}
